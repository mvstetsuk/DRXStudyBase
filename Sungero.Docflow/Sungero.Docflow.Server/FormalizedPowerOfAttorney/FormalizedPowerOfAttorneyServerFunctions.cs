...
namespace Sungero.Docflow.Server
{
  partial class FormalizedPowerOfAttorneyFunctions
  {
    ...
    public virtual void FillRegistrationData(string number, DateTime? date)
    {
      ...
      {
        var matchingRegisters = Functions.OfficialDocument.GetDocumentRegistersByDocument(_obj, Docflow.RegistrationSetting.SettingType.Numeration);
        if (matchingRegisters.Count == 1)
        {
          Functions.OfficialDocument.RegisterDocument(_obj, matchingRegisters.FirstOrDefault(), date, number, false, false);
          return;
        }
      }
      ...
      else
      {
        ...
        _obj.Note = _obj.Note == Constants.Module.ZeroWidthSpace.ToString() ?
          registrationDataString :
          registrationDataString + Environment.NewLine + _obj.Note;
      }
      return;
    }
