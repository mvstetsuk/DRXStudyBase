        ...
        b =>
        {
          if (b.Button == importButton && b.IsValid)
          {
            try
            {
              ...
              // TODO: Убрать предзаполнение Примечания на клиенте и соответствующую проверку на сервере (Bug 193156).
              if (string.IsNullOrEmpty(userNote))
                _obj.Note = Constants.Module.ZeroWidthSpace.ToString();
